...
// [핵심 수정] 돛 상태 변경 시 물리적 브레이크 적용
    public void setSailState(int state) {
        // 1. 관성 제거 (워터 브레이크)
        // Controller가 정지(0)를 요청하면, 보트의 물리 속도 벡터를 강제로 줄입니다.
        // 쿨타임 여부와 상관없이 즉시 반응해야 미끄러지지 않습니다.
        if (state == 0) {
            Vec3 motion = boat.getDeltaMovement();
            // X, Z(수평) 속도는 5%만 남기고 삭제 (강력한 제동), Y(수직)는 유지하여 침몰 방지
            boat.setDeltaMovement(motion.multiply(0.05, 1.0, 0.05));
        }

        // 2. 기존 SmallShips 모드 내부 로직 호출 (돛 모양 변경 등)
        try {
            Class<?> shipClass = Class.forName("com.talhanation.smallships.world.entity.ship.Ship");
            if (shipClass.isInstance(boat)) {
	...
	// [수정] 인자에 'double speed'를 추가했습니다.
	public static void shootCannonsSmallShip(CaptainEntity driver, Boat boat, Entity target, boolean leftSide, double speed) {
		if (boat == null || target == null || driver == null) return;

		// [삭제] NBT 읽기, Modifier 계산 등 불필요한 로직 전부 삭제
		// float basePower = ... (삭제)
		// Modifier 더하는 코드 ... (삭제)

		double accuracy = 2.0F;

		Vec3 shootVec = target.position().subtract(boat.position()).normalize();
		double yShootVec = shootVec.y(); 
		...
		try {
			...
			if (cannonAbleClass.isInstance(boat)) {
				...
				// [중요] 인자로 받은 'speed' (4.5)를 그대로 리플렉션에 전달
